const WhatsAppModule = (function() {
  ...
  function checkForAudioMessages() {
    // Buscar todos los mensajes de audio
    const audioMessages = document.querySelectorAll('[data-testid="audio-play"]');

    audioMessages.forEach(audioMsg => {
      const container = audioMsg.closest('[data-testid="msg-container"]') ||
                       audioMsg.closest('div[class*="message"]');

      if (container && !container.querySelector('.ai-whatsapp-transcribe-btn')) {
        console.log('🎤 WhatsApp: Audio detectado, insertando botón de transcripción');
        insertTranscribeButton(container, audioMsg);
      }
    });
  }
   ...
   */
  function insertTranscribeButton(container, audioElement) {
    const btn = document.createElement('button');
    btn.className = 'ai-whatsapp-transcribe-btn';
    btn.setAttribute('aria-label', 'Transcribir audio');
    btn.setAttribute('type', 'button');
    ...
    btn.addEventListener('click', async (e) => {
      ...
      await handleTranscribeAudio(audioElement, btn, container);
    });

    // Insertar el botón al lado del audio
    const audioContainer = audioElement.closest('div[role="button"]') || audioElement.parentElement;
    if (audioContainer) {
      audioContainer.style.position = 'relative';
      audioContainer.appendChild(btn);
    }

    console.log('✅ WhatsApp: Botón de transcripción insertado');
  }
  ...
  async function handleTranscribeAudio(audioElement, buttonElement, container) {
    try {
      ...
      // Intentar obtener el audio
      // Nota: WhatsApp Web encripta los audios, así que necesitaríamos acceso directo al blob
      // Por ahora, mostraremos un diálogo para que el usuario pueda grabar o subir audio

      // Crear diálogo de transcripción
  ...
  function createTranscribeDialogHTML() {
    return `
      <header class="ai-result-header ai-draggable">
        ...
        <div class="title">🎤 Transcribir Audio</div>
      ...
      <div class="ai-result-body" style="padding: 1.5rem;">
        <div class="ai-whatsapp-transcribe-section">
          <div class="transcribe-options">
            <button class="transcribe-btn record-btn" data-action="record">
              <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
                <circle cx="12" cy="12" r="8"/>
              </svg>
              Grabar audio
            </button>
            <button class="transcribe-btn upload-btn" data-action="upload">
          ...
          <div class="audio-player-section" style="display: none; margin-top: 1rem;">
            ...
            <select class="transcribe-mode">
              <option value="transcribe">Transcribir texto</option>
